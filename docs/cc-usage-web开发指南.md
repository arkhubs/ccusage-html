...
- [x] 模块化代码，要高内聚低耦合
	...
	- [x] 3. 页面全挤在一起了，可以做成分 tab 的布局。
		...
		- [x] 但是现在 session 页面显得很乱，需要改进：
			...
			- [ ] gemini 会话的时间是 unkown，有办法修复吗
			- [ ] 目前 session 的排序还是假的，初始也没有按时间倒序
			- [ ] 会话聊天记录可以改成允许 load more 的动态加载吗？
	- [ ] 现在只能从本地文件 html 点击启动，点给的网址打不开。是不是沙箱运行的缘故？
		- [ ] 而且我需要稳定触发快速启动，快速拿到网址。现在太慢了
